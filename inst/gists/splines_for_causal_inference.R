...
# Non-Linear Relationship Between Z1 & Y ----

# Note: in this example, we could just use the actual formula and fit a model 
# using z1^3 + z1, however in reality we don't actually know the answer. 

set.seed(456)

df <- sim_data() %>% 
  mutate(
    y = 1.5*x + z1^3 + z1 + z2 # changing y to have non-linear relationship with z1
  )
